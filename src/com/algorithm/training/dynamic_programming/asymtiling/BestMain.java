...
public class BestMain {
	...
	public int getAsymmetricCount(int width) {
		...
		if(width % 2 == 0) {
			asymmetricCache[width-1] = (tilingCount - getTilingCount((width-2)/2) - getTilingCount(width/2)) % MOD_VALUE;
		} else {
			asymmetricCache[width-1] = (tilingCount - getAsymmetricCount(width/2)) % MOD_VALUE;
		}
